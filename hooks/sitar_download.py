...
that contains a --8<-- snippet import from docs_source/sitar_examples/*.sitar.
...
# include from docs_source/sitar_examples/FNAME.sitar (with optional :section suffix).
_FENCE_RE = re.compile(
    r'(?m)'
    r'^(?P<open>`{3,}[ \t]*sitar[^\n]*\n)'               # opening fence line
    r'(?P<include>[ \t]*--8<--[ \t]*"docs_source/sitar_examples/'
