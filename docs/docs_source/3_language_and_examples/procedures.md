...
``` sitar linenums="1"
--8<-- "docs_source/sitar_examples/3_procedures.sitar:basic"
```
...
For the parameterized and nested procedure examples, see `docs_source/sitar_examples/3_procedures.sitar` sections `:parameterized` and `:nested`.
