...
The file `docs_source/sitar_examples/3_logging.sitar` contains named sections demonstrating each logging feature covered above. The sections `:basic`, `:multiple_loggers`, `:prefix`, and `:control` can be read independently.

``` sitar linenums="1"
--8<-- "docs_source/sitar_examples/3_logging.sitar:basic"
```
